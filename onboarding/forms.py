...
class EmployeeCreationForm(ModelForm):
    ...
    def clean(self):
        ...
        if cleaned_data['experience'] and cleaned_data['experience'] <0:
            raise ValidationError({'experience':_('Experience should be a postive integier')})
        if cleaned_data['children'] and cleaned_data['children'] <0:
            raise ValidationError({'children':_('No of children should be a postive integier')})
