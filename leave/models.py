    ...
    def clean(self):
        ...
        if request.user.is_superuser:
            ...
            for restrict in restricted_leave:
                ...
                if restrict.include_all and len(restrict.exclued_leave_types.all()) == 0:
...
class RestrictLeave(HorillaModel):
    ...
    include_all = models.BooleanField(default=True, help_text=_("Enable to select all Leave types."))
    spesific_leave_types = models.ManyToManyField(LeaveType, verbose_name=_("Spesific leave types"), 
                                                related_name="spesific_leave_type", blank=True,
                                                help_text=_("Choose specific leave types to restrict."),)
    exclued_leave_types = models.ManyToManyField(LeaveType, verbose_name=_("Exclude leave types"), 
                                                related_name="excluded_leave_type", blank=True,
...
if apps.is_installed("attendance"):
    ...
    class CompensatoryLeaveRequest(HorillaModel):
        ...
        attendance_id = models.ManyToManyField(
            "attendance.Attendance", verbose_name="Attendance",
        )
        requested_days = models.FloatField(blank=True, null=True)
        requested_date = models.DateField(default=timezone.now)
        description = models.TextField(max_length=255)
