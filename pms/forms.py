...
from base.methods import filtersubordinatesemployeemodel, is_reportingmanager, reload_queryset
...
class EmployeeObjectiveCreateForm(BaseForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if request.user.has_perm('pms.add_employeeobjective') or is_reportingmanager(request):
            employees = filtersubordinatesemployeemodel(
                request,
                queryset=Employee.objects.filter(is_active = True),
                perm="pms.add_employeeobjective"
            )
            self.fields['employee_id'].queryset = employees | Employee.objects.filter(employee_user_id=request.user)       
...
class MeetingsForm(BaseForm):
    ...
    def clean(self):
        ...
        if cleaned_data["answer_employees"] and not cleaned_data["question_template"]:
            raise ValidationError({"question_template": _("Question template is required when answer employees are choosed")})
    ...
    def clean(self):
        ...
        if model in ['pms.models.EmployeeObjective','pms.models.EmployeeKeyResult']:
            if  not cleaned_data.get('applicable_for') == 'owner':              
                raise ValidationError(_(f"Model Doesn't have this {cleaned_data.get('applicable_for')} field"))
            if not cleaned_data['bonus_for'] == 'Closed':
                raise ValidationError(_(f"This 'Bonus for' is not in the Model's status"))        
