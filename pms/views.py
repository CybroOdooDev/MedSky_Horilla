...
from pms.methods import check_permission_feedback_detailed_view,pms_owner_and_manager_can_enter
...
def feedback_list_search(request):
    ...
    all_feedback = Feedback.objects.filter(Q(manager_id=employee_id, manager_id__is_active=True,archive=False,)).filter(review_cycle__icontains=feedback)
    anonymous_feedback = (
...
def feedback_detailed_view(request, id, **kwargs):
    ...
    """
    feedback = Feedback.objects.get(id=id)
    is_have_perm = check_permission_feedback_detailed_view(request,feedback,"pms.view_Feedback")
    if is_have_perm:
        ...
        for employee in employees:
            if Answer.objects.filter(feedback_id=feedback,employee_id = employee).exists():
                yes.append(employee)
            else:
                no.append(employee)
        employee_statics ={
    ...
    try:
        ...
        if employee:
            ...
            if request.GET.get('data') == 'colleagues':
                ...
                reporting_manager = employee.employee_work_info.reporting_manager_id if employee.employee_work_info else None
