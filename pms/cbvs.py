...
@method_decorator(
    permission_required("pms.view_bonuspointsetting"), name="dispatch"
)
...
@method_decorator(login_required, name="dispatch")
@method_decorator(
    permission_required("pms.change_bonuspointsetting"), name="dispatch"
)
...
class EmployeeBonusPointNavView(views.HorillaNavView):
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        if request:
            if is_reportingmanager(request) or request.user.has_perm("pms.add_employeebonuspoint"):
                self.create_attrs = f"""
...
@method_decorator(login_required, name="dispatch")
@method_decorator(
    permission_required("pms.change_employeebonuspoint"), name="dispatch"
)
...
class EmployeeBonusPointListView(views.HorillaListView):
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        request = self.request
        if request:
            if is_reportingmanager(request) or request.user.has_perm("pms.change_employeebonuspoint"):
    ...
    def get_queryset(self):
        ...
        if is_reportingmanager(request) or request.user.has_perm("pms.view_employeebonuspoint"):
            return filter_own_and_subordinate_recordes(request, queryset, perm='pms.view_employeebonuspoint')
        else:
