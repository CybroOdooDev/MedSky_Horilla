    ...
    def start_connection():
        ...
        for bonus_point_setting in bonus_point_settings:
            ...
            for field_name,relation_type in related_fields:
                if bonus_point_setting.applicable_for == 'members' and relation_type == 'ManyToManyField':
                        field = field_name
                        type = relation_type
                        break
                elif bonus_point_setting.applicable_for == 'managers'or bonus_point_setting.applicable_for== 'owner' and relation_type == 'ForeignKey':
                        field = field_name
                        type = relation_type
                        break
                ...
                def signal_handler(sender,instance,*args,**kwargs):
                    ...
                    if type == 'ManyToManyField':
                        ...
                        def members_changed(sender, instance, action, **kwargs):
                            ...
                            """
                            if action == 'post_add' or action == 'post_remove' or action == 'post_clear' or action == 'post_save':
                                ...
                                if bonus_point_setting.bonus_for == instance.status:
                                    if field and type:
                                        ...
                                        for employee in employees:
                                            bonus_point_setting.create_employee_bonus(employee, field_1, field_2,instance)
                                    else :
            ...
            dynamic_signal_handler = create_signal_handler(handler_name,bonus_point_setting,type=type,field=field)
            SIGNAL_HANDLERS.append(dynamic_signal_handler)
            post_save.connect(dynamic_signal_handler, sender=dynamic_signal_handler.model_class)
