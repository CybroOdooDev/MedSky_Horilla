                                ...
                                if bio_id:
                                    last_none_activity = (
                                        AttendanceActivity.objects.filter(
                                            employee_id = bio_id.employee_id, clock_out = None
                                        )
                                        .order_by("in_datetime").last())
                                    print(last_none_activity)
...
def zk_biometric_device_attendance(device_id):
    ...
    if device.is_scheduler:
        ...
        try:
            ...
            last_attendance_datetime = None
            for attendance in reversed(attendances):
                if attendance.timestamp is not None and attendance.user_id is not None:
                    last_attendance_datetime = attendance.timestamp
                    break
            ...
            device.last_fetch_date = last_attendance_datetime.date() if last_attendance_datetime else None
            device.last_fetch_time = last_attendance_datetime.time() if last_attendance_datetime else None
            device.save()
            for attendance in attendances:
