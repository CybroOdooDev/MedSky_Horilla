...
def interview_accessibility(
    ...
    if getattr(request.user,"employee_get",None) and request.user.employee_get in interviewers:
        view_interview = True
    else:
...
def skill_zone_accessibility(
    ...
    return is_stagemanager(request.user) or request.user.has_perm("recruitment.view_skillzone")
